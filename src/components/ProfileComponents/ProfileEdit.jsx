...
const ProfileEdit = () => {
  ...
  const [selectedDate, setSelectedDate] = useState(parseDate(current.birthDate));
  const [maritalStatus, setMaritalStatus] = useState(current.maritalStatus);
  const [gender, setGender] = useState(current.gender);
  const [division, setDivision] = useState([current.divisionResponse.divisionId,current.divisionResponse.name]);
...
<div className='d-flex flex-column w-100'>
  <form onSubmit={handleSubmit(onSubmit)} className="shadow-sm p-4 rounded-2 bg-white">
    ...
    <div className="mb-2">
      ...
      <div className="dropdown">
        ...
        <div className="dropdown-menu" aria-labelledby="maritalStatus" >
          <a className="dropdown-item" onClick={()=>setMaritalStatus("MARRIED")}>MARRIED</a>
          <a className="dropdown-item" onClick={()=>setMaritalStatus("SINGLE")}>SINGLE</a>
        </div>
      </div>
    </div>
    <div className="mb-2">
    ...
    <div className="mb-2">
      ...
      <input {...register("email")} className={`bg-light form-control rounded-3 border-0 border-bottom ${errors.email && "is-invalid"}`} disabled={true} type="email" name="email" />
