...
function UserService() {
  const createAdmin = async (payload) => {
    console.log(payload);
    try {
      const response = await proplyInstance.post(
        "/auth/register/admin",
        payload
      );
  ...
  const getAll = async () => {
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Fetch data users failed"
      );
    }
  };
  ...
  const getCurrentUser = async (payload) => {
    try {
      ...
      if (response.data.statusCode === 200) {
        ...
        throw new Error(
          response.data.message || "Fetch data current user failed"
        );
      }
    } catch (error) {
      throw new Error(
        error.response?.data?.message || "Fetch data current user failed"
      );
    }
  };
  ...
  const remove = async (payload) => {
    try {
      const response = await proplyInstance.delete(
